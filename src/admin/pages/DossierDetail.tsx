/**
 * =============================================================================
 * PAGE DÉTAIL DOSSIER — Interface Prestataire
 * Layout optimisé : Infos client à GAUCHE | Chat IA à DROITE
 * 
 * FIX V5.1 : Suppression de l'appel API aiChat pour éviter les doublons
 *            Le trigger Firestore aiOnProviderMessage gère seul les réponses IA
 * =============================================================================
 */
        ...
        {messages.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h4 className="font-medium text-gray-900 mb-2">
              {t("aiChat.expertTitle")}
            </h4>
            <p className="text-sm text-gray-600 max-w-xs mx-auto">
              {t("aiChat.expertDescription")}
            </p>
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {/* Actions pour les réponses IA */}
                  {isAI && !isError && (
                    <div className="mt-3 pt-2 border-t border-gray-100 flex justify-end">
                      <button
                        onClick={() => copyToClipboard(message.content, message.id)}
                        className="inline-flex items-center gap-1.5 px-3 py-2 min-h-[44px] rounded-lg hover:bg-gray-100 transition-colors text-xs text-gray-600 touch-scale-down"
        ...
        {disabled ? (
          <div className="bg-gray-100 rounded-xl p-4 text-center">
            ...
            <p className="text-sm text-gray-500 mt-1">
              {t("dossierDetail.historyAvailable")}
            </p>
          </div>
        ) : (
          <>
            <form onSubmit={handleSubmit} className="flex gap-3">
              ...
              <button
                ...
                className="px-5 py-3 min-h-[48px] min-w-[48px] bg-purple-600 text-white rounded-xl hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2 touch-scale-down"
              >
                {isLoading ? (
    ...
    <div className="space-y-4 overflow-y-auto">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        <div className="flex items-center justify-between mb-4">
          {/* Indicateur d'état basé sur le temps */}
            ...
            {isExpired ? (
              <>
                ...
                <p className="text-sm font-medium text-gray-600">
                  {t("dossierDetail.consultationTimeElapsed")}
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  {t("clientInfo.duration", { minutes: durationMinutes })}
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {t("clientInfo.timeRemaining", { minutes: durationMinutes })}
                </p>
              </>
            )}
          </div>
...
export default function DossierDetail() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (docSnap) => {
        if (docSnap.exists()) {
          ...
          // ═══════════════════════════════════════════════════════════════════
          // SÉCURITÉ: Vérifier que l'utilisateur a le droit d'accéder à ce dossier
          // - Admin peut accéder à tous les dossiers
          // - Prestataire peut accéder uniquement à ses propres dossiers
          // ═══════════════════════════════════════════════════════════════════
          if (!isAdmin && bookingData.providerId && !booking) {
            // Vérifier si le providerId du booking est dans les providers liés de l'utilisateur
            const hasAccess = linkedProviders.some(p => p.id === bookingData.providerId) ||
                              activeProvider?.id === bookingData.providerId;

            if (!hasAccess) {
              console.warn("[DossierDetail] Accès refusé - Provider non autorisé:", {
                bookingProviderId: bookingData.providerId,
                activeProviderId: activeProvider?.id,
                linkedProviderIds: linkedProviders.map(p => p.id)
              });
    ...
    return () => unsubscribe();
  }, [id, isAdmin, activeProvider?.id, linkedProviders, booking]);

  // Charger/créer la conversation et écouter les messages
  // FIX: Utiliser une requête WHERE pour trouver la conversation existante
  useEffect(() => {
    ...
    const loadConversation = async () => {
      // Chercher une conversation existante pour ce booking avec WHERE
      ...
      if (!convSnapshot.empty) {
        // Conversation existante trouvée - utiliser la première
        const existingConv = convSnapshot.docs[0];
        convId = existingConv.id;
        setConversation({ id: convId, ...existingConv.data() } as Conversation);
        console.log("[DossierDetail] Conversation existante trouvée:", convId);
      } else {
        // Pas de conversation - en créer une nouvelle
        console.log("[DossierDetail] Création nouvelle conversation pour booking:", id);
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // FIX V5.1 : Envoyer un message - TRIGGER FIRESTORE UNIQUEMENT (pas d'API)
  // Le trigger aiOnProviderMessage se charge de générer la réponse IA
  // ═══════════════════════════════════════════════════════════════════════════
  const handleSendMessage = useCallback(async (message: string) => {
    ...
    try {
      // Ajoute le message dans Firestore
      // Le trigger aiOnProviderMessage va automatiquement :
      // 1. Détecter ce nouveau message (source: "provider")
      // 2. Appeler GPT-4o + Perplexity
      // 3. Ajouter la réponse IA dans la conversation
      ...
      // La réponse IA arrivera automatiquement via le listener onSnapshot
      // Le loading sera désactivé quand la réponse arrive (voir useEffect ci-dessus)
  ...
  return (
    <div className="h-[calc(100vh-120px)] flex flex-col">
      ...
      {/* Layout principal : Gauche (infos) | Droite (chat) - avec min-height pour éviter les layout shifts */}
      <div
      ...
      >
        {/* Panneau gauche - Infos client (masqué avec transition au lieu de supprimé) */}
        <div
