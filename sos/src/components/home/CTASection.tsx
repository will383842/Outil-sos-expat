        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl sm:rounded-3xl p-8 sm:p-12 lg:p-16">
            ...
            >
              <span className="flex items-center justify-center gap-3">
                ...
                <span className="whitespace-nowrap">
                  {intl.formatMessage({ id: 'cta.startNow' })}
                </span>
          ...
          <div className="flex flex-wrap justify-center gap-6 sm:gap-8 text-white/70 text-sm sm:text-base">
            <span className="flex items-center gap-2 hover:text-white transition-colors">
              ...
              {intl.formatMessage({ id: 'cta.noCommitment' })}
            </span>
            <span className="flex items-center gap-2 hover:text-white transition-colors">
              <Shield className="h-4 w-4 sm:h-5 sm:w-5 text-blue-400" />
              {intl.formatMessage({ id: 'cta.secure' })}
            </span>
            <span className="flex items-center gap-2 hover:text-white transition-colors">
              <Zap className="h-4 w-4 sm:h-5 sm:w-5 text-yellow-400" />
              {intl.formatMessage({ id: 'cta.instantActivation' })}
            </span>
          </div>

          <div className="mt-8 sm:mt-12 grid grid-cols-3 gap-4 sm:gap-8 max-w-2xl mx-auto">
            {[
              { icon: Award, text: intl.formatMessage({ id: 'cta.numberOne' }) },
              { icon: ShieldCheck, text: intl.formatMessage({ id: 'cta.dataProtected' }) },
              { icon: Star, text: intl.formatMessage({ id: 'cta.rating' }) }
            ].map((item, i) => (
