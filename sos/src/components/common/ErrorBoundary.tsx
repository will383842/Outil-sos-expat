        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              >
                {this.props.intl.formatMessage({ id: 'error.boundary.title' })}
              </h3>
              <div
              ...
              >
                <p>
                  {this.props.intl.formatMessage({ id: 'error.boundary.description' })}
                </p>
                <p>
                  {this.props.intl.formatMessage({ id: 'error.boundary.reported' })}
                </p>
                {this.state.eventId && (
                  <p className="text-xs text-red-600">
                    {this.props.intl.formatMessage({ id: 'error.boundary.reference' })} : {this.state.eventId}
                  </p>
                )}
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2 mt-4">
                <button
                  ...
                  aria-label={this.props.intl.formatMessage({ id: 'error.boundary.retryLabel' })}
                >
                  <RefreshCw className="w-4 h-4 mr-2" aria-hidden="true" />
                  {this.props.intl.formatMessage({ id: 'error.boundary.retry' })}
                </button>
                <button
                  ...
                  aria-label={this.props.intl.formatMessage({ id: 'error.boundary.reloadLabel' })}
                >
                  {this.props.intl.formatMessage({ id: 'error.boundary.reload' })}
                </button>
              </div>
              ...
              {process.env.NODE_ENV === 'development' && this.state.error && (
                <details className="mt-6 p-3 bg-gray-100 rounded-md">
                  <summary className="text-sm font-medium text-gray-700 cursor-pointer hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 rounded">
                    {this.props.intl.formatMessage({ id: 'error.boundary.devDetails' })}
                  </summary>
                  <div className="mt-3 p-3 bg-gray-900 text-gray-100 rounded-md overflow-auto">
                    <div className="text-xs font-mono space-y-2">
                      <div>
                        <strong className="text-red-400">{this.props.intl.formatMessage({ id: 'error.boundary.errorLabel' })}</strong>
                        <p className="mt-1 text-red-300">{this.state.error.toString()}</p>
                      </div>
                      {this.state.error.stack && (
                        <div>
                          <strong className="text-yellow-400">{this.props.intl.formatMessage({ id: 'error.boundary.stackTrace' })}</strong>
                      ...
                      {this.state.errorInfo?.componentStack && (
                        <div>
                          <strong className="text-blue-400">{this.props.intl.formatMessage({ id: 'error.boundary.componentStack' })}</strong>
                          <pre className="mt-1 text-gray-300 whitespace-pre-wrap break-all">
                            {this.state.errorInfo.componentStack}
                          </pre>
                        </div>
