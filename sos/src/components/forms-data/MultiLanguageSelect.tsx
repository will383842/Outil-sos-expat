...
import { getNoOptionsMessage, defaultPlaceholderByLocale, Locale } from './shared';
...
interface MultiLanguageSelectProps {
  ...
  locale?: 'fr' | 'en' | 'es' | 'de' | 'ru' | 'hi' | 'ch' | 'pt' | 'ar';
  placeholder?: string;
  className?: string;
  disabled?: boolean;
}
...
const MultiLanguageSelect: React.FC<MultiLanguageSelectProps> = React.memo(({ 
  ...
  const currentLocale = useMemo<'fr' | 'en' | 'es' | 'de' | 'ru' | 'hi' | 'ch' | 'pt' | 'ar'>(() => {
  ...
  const noOptionsMessage = useCallback(({ inputValue }: { inputValue: string }) => {
    return getNoOptionsMessage(currentLocale as Locale, inputValue);
  }, [currentLocale]);

  // Placeholder par défaut
  const defaultPlaceholder = useMemo(() => {
    return defaultPlaceholderByLocale[currentLocale as Locale] || defaultPlaceholderByLocale.en;
  }, [currentLocale]);
