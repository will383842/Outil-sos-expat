...
export const CurrencySelector: React.FC<CurrencySelectorProps> = ({
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      <h4 className="text-sm font-medium text-gray-700">{intl.formatMessage({ id: 'checkout.currency.label' })}</h4>
          ...
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center space-x-2">
              ...
              <span className={`font-medium text-sm ${selectedCurrency === 'eur' ? 'text-blue-900' : 'text-gray-900'}`}>
                {intl.formatMessage({ id: 'checkout.currency.eur' })}
              </span>
            </div>
            {selectedCurrency === 'eur' && (
          ...
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center space-x-2">
              ...
              <span className={`font-medium text-sm ${selectedCurrency === 'usd' ? 'text-green-900' : 'text-gray-900'}`}>
                {intl.formatMessage({ id: 'checkout.currency.usd' })}
              </span>
            </div>
            {selectedCurrency === 'usd' && (
      ...
      <div className="bg-gray-50 rounded-lg p-3">
        <p className="text-xs text-gray-600">
          {intl.formatMessage({ id: 'checkout.currency.autoDetectHint' })}
        </p>
      </div>
    </div>
  );
