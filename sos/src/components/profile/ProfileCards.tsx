  ...
  const transformFirestoreDoc = useCallback((doc: FirebaseDocumentSnapshot): Provider | null => {
    try {
      ...
      const languages = Array.isArray(data.languages) && data.languages.length > 0
        ...
        : [intl.formatMessage({ id: 'profile.defaultLanguage' })];
...
// Règle globale d'affichage: prestataires visibles même sans KYC complété
// - Exclus : admin + bannis + non visibles
// - isApproved NON requis pour permettre aux prestataires sans KYC d'être visibles
filtered = filtered.filter(p => {
  const notAdmin = (p.role ?? '') !== 'admin' && p.isAdmin !== true;
  const notBanned = p.isBanned !== true;
  const visible = p.isVisible !== false;
  return notAdmin && notBanned && visible;
});
    // Base filters with AI-friendly logic - isApproved non requis
    if (filter === 'providers-only') {
      filtered = filtered.filter(provider =>
        provider.type === 'expat' || provider.type === 'lawyer'
      );
