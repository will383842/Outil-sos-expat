/**
 * AffiliateToolsContent - Inner content for the marketing tools page
 * Reusable by AffiliateTools, InfluencerTools, BloggerTools (each wraps with its own layout)
 *
...
const AffiliateToolsContent: React.FC = () => {
  const intl = useIntl();
  ...
  const applyTemplate = (template: typeof CAMPAIGN_TEMPLATES[0]) => {
    setUtmParams(prev => ({ ...prev, campaign: template.utm_campaign }));
  };
    ...
    <div className="space-y-6">
      ...
      <div className={`${UI.card} p-6`}>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
          ...
          <a href={socialShareUrls.whatsapp} target="_blank" rel="noopener noreferrer"
          ...
          <a href={socialShareUrls.telegram} target="_blank" rel="noopener noreferrer"
            className="flex flex-col items-center gap-2 p-4 rounded-xl bg-sky-50 dark:bg-sky-900/20 hover:bg-sky-100 dark:hover:bg-sky-900/30 transition-colors">
          ...
          {[1, 2, 3, 4].map((n) => (
            <li key={n} className="flex items-start gap-3">
              ...
              <span className="text-sm text-gray-600 dark:text-gray-400">
                <FormattedMessage id={`affiliate.tools.tips.${n}`} defaultMessage="" />
              </span>
            </li>
          ))}
        </ul>
...
export default AffiliateToolsContent;
