...
const GroupAdminLanding: React.FC = () => {
  ...
  const [gaReleases, setGaReleases] = useState<Array<{ id: string; title: Record<string, string>; htmlUrl?: Record<string, string>; pdfUrl?: Record<string, string>; tags?: string[]; publishedAt?: Date }>>([]);

  const loadGaReleases = useCallback(async () => {
    try {
      const snap = await getDocs(query(collection(db, 'group_admin_releases'), where('isActive', '==', true), orderBy('publishedAt', 'desc')));
      setGaReleases(snap.docs.map(d => ({ id: d.id, ...(d.data() as any), publishedAt: d.data().publishedAt?.toDate?.() })));
    } catch { /* silent */ }
  }, []);

  useEffect(() => { loadGaReleases(); }, [loadGaReleases]);
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid lg:grid-cols-3 gap-5 sm:gap-6 lg:gap-8">
              ...
              <article className="bg-gradient-to-br from-green-500/20 to-emerald-500/10 border rounded-2xl sm:rounded-3xl p-5 sm:p-6 lg:p-8">
                ...
                <div className="text-sm font-bold mb-2">
                  <FormattedMessage id="groupAdmin.landing.step2.label" defaultMessage="ÉTAPE 2" />
                </div>
                <h3 className="!text-2xl sm:!text-2xl lg:!text-3xl font-bold text-white mb-3 sm:mb-4">
                  <FormattedMessage id="groupAdmin.landing.step2.title" defaultMessage="Épinglez & Partagez" />
                </h3>
                <p className="text-base sm:text-lg lg:text-xl">
                  <FormattedMessage id="groupAdmin.landing.step2.description" defaultMessage="Choisissez un visuel dans votre dashboard, épinglez-le dans votre groupe, et partagez vos posts épinglés avec votre lien affilié intégré. Ressources disponibles en FR · EN · ES · PT · AR · DE · IT · NL · ZH !" />
                </p>
              </article>
              ...
              <article className="bg-gradient-to-br from-yellow-500/20 to-amber-500/10 border rounded-2xl sm:rounded-3xl p-5 sm:p-6 lg:p-8">
                ...
                <div className="text-sm font-bold mb-2">
                  <FormattedMessage id="groupAdmin.landing.step3.label" defaultMessage="ÉTAPE 3" />
                </div>
                <h3 className="!text-2xl sm:!text-2xl lg:!text-3xl font-bold text-white mb-3 sm:mb-4">
              ...
              <div className="bg-gradient-to-br from-blue-500/15 to-indigo-500/10 border rounded-2xl sm:rounded-3xl p-5 sm:p-6 lg:p-8">
                ...
                <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-xl p-5 text-center">
                  ...
                  <div className="text-xs text-blue-300 mb-3">
                    ≈ {Math.round(groupMembers * conversionRate / 100)} <FormattedMessage id="groupAdmin.landing.calc.calls" defaultMessage="appels/mois estimés" />
                  </div>
          ...
          <section className="section-content" aria-label="Communiqué admin groupes">
            <div className="max-w-5xl mx-auto">
              ...
              <p className="text-center text-sm mb-8 opacity-60">
                <FormattedMessage id="groupAdmin.press.subtitle" defaultMessage="Téléchargez et partagez notre kit presse" />
              </p>
              <div className={`grid gap-6 ${gaReleases.length === 1 ? 'max-w-sm mx-auto' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'}`}>
                {gaReleases.map(release => {
                  ...
                  return (
                    <article key={release.id} className="group flex flex-col rounded-2xl overflow-hidden border border-white/10 hover:border-blue-500/40 transition-all duration-300 bg-white/[0.03]">
                      ...
                      <div className="p-4 flex flex-col gap-2">
                        <div className="flex flex-wrap gap-1.5">
                          {release.tags?.slice(0, 3).map(tag => (
                            <span key={tag} className="px-2 py-0.5 bg-blue-600/15 text-blue-400 text-[10px] rounded-full font-semibold">{tag}</span>
                          ))}
                        </div>
