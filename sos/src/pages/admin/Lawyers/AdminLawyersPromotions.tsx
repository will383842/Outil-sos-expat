/**
 * AdminLawyersPromotions
 *
 * Admin page for managing lawyer promotions/hackathons.
...
const COMMISSION_TYPES = [
  ...
  { value: "provider_recruitment", label: "Provider Recruitment" },
];
...
const AdminLawyersPromotions: React.FC = () => {
  ...
  const fetchPromotions = useCallback(async () => {
    ...
    setError(null);
    try {
      const fn = httpsCallable(functionsAffiliate, "adminGetLawyerPromotions");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingPromo) {
        const fn = httpsCallable(functionsAffiliate, "adminUpdateLawyerPromotion");
        await fn({
        ...
        const fn = httpsCallable(functionsAffiliate, "adminCreateLawyerPromotion");
        await fn({
          name: formData.name,
          description: formData.description,
          type: formData.type,
  ...
  const handleDelete = async (promoId: string) => {
    ...
    try {
      const fn = httpsCallable(functionsAffiliate, "adminDeleteLawyerPromotion");
  ...
  const handleDuplicate = async (promo: Promotion) => {
    try {
      const fn = httpsCallable(functionsAffiliate, "adminDuplicateLawyerPromotion");
      await fn({
                ...
                <TableBody>
                  {promotions.map((promo) => (
                    <TableRow key={promo.id}>
                      ...
                      <TableCell>
                        <span className="font-bold text-amber-600">x{promo.multiplier}</span>
                      </TableCell>
                      <TableCell>
...
export default AdminLawyersPromotions;
