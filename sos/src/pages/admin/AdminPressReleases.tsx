...
const AdminPressReleases: React.FC = () => {
  ...
  const loadReleases = useCallback(async () => {
    try {
      setIsLoading(true);
      const q = query(collection(db, "press_releases"), orderBy("publishedAt", "desc"));
  ...
  const handleToggleActive = async (release: PressRelease) => {
    try {
      await updateDoc(doc(db, "press_releases", release.id), {
        isActive: !release.isActive,
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, "press_releases", showDeleteConfirm.id));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let docId: string;
      if (isCreating) {
        const newDocRef = await addDoc(collection(db, "press_releases"), {
          ...baseDocData,
      ...
      if (imageFile) {
        ...
        const storagePath = `press/releases/${docId}/image.${ext}`;
      ...
      for (const [langCode, file] of Object.entries(pdfFiles)) {
        if (file) {
          const storagePath = `press/releases/${docId}/pdfs/${langCode}.pdf`;
          const storageRef = ref(storage, storagePath);
  ...
  return (
    <AdminLayout>
      <div className="px-4 sm:px-6 lg:px-8 py-8">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 text-gray-500">
            ...
            <p>Aucun communiqué de presse</p>
          </div>
        ) : (
      ...
      <Modal
        ...
        title={isCreating ? "Nouveau communiqué de presse" : "Modifier le communiqué"}
        size="large"
      >
