/**
 * AdminClientsPromotions
 *
 * Admin page for managing client promotions/hackathons.
...
const AdminClientsPromotions: React.FC = () => {
  ...
  const fetchPromotions = useCallback(async () => {
    ...
    setError(null);
    try {
      const fn = httpsCallable(functionsAffiliate, "adminGetClientPromotions");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingPromo) {
        const fn = httpsCallable(functionsAffiliate, "adminUpdateClientPromotion");
        await fn({
        ...
        const fn = httpsCallable(functionsAffiliate, "adminCreateClientPromotion");
        await fn({
          name: formData.name,
          description: formData.description,
          type: formData.type,
  ...
  const handleDelete = async (promoId: string) => {
    ...
    try {
      const fn = httpsCallable(functionsAffiliate, "adminDeleteClientPromotion");
  ...
  const handleDuplicate = async (promo: Promotion) => {
    try {
      const fn = httpsCallable(functionsAffiliate, "adminDuplicateClientPromotion");
      await fn({
                ...
                <TableBody>
                  {promotions.map((promo) => (
                    <TableRow key={promo.id}>
                      ...
                      <TableCell>
                        <span className="font-bold text-emerald-600">x{promo.multiplier}</span>
                      </TableCell>
                      <TableCell>
...
export default AdminClientsPromotions;
