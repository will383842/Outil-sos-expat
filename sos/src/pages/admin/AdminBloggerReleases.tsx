...
const AdminBloggerReleases: React.FC = () => {
  ...
  const loadReleases = useCallback(async () => {
    try {
      setIsLoading(true);
      const q = query(collection(db, "blogger_releases"), orderBy("publishedAt", "desc"));
  ...
  const handleToggleActive = async (release: PressRelease) => {
    try {
      await updateDoc(doc(db, "blogger_releases", release.id), {
        isActive: !release.isActive,
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, "blogger_releases", showDeleteConfirm.id));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let docId: string;
      if (isCreating) {
        const newDocRef = await addDoc(collection(db, "blogger_releases"), {
          ...baseDocData,
      ...
      if (imageFile) {
        ...
        const storagePath = `blogger/releases/${docId}/image.${ext}`;
      ...
      for (const [langCode, file] of Object.entries(pdfFiles)) {
        if (file) {
          const storagePath = `blogger/releases/${docId}/pdfs/${langCode}.pdf`;
          const storageRef = ref(storage, storagePath);
      ...
      for (const [langCode, file] of Object.entries(htmlFiles)) {
        if (file) {
          const storagePath = `blogger/releases/${docId}/html/${langCode}.html`;
          const storageRef = ref(storage, storagePath);
      ...
      if (hasUploads || !isCreating) {
        await updateDoc(doc(db, "blogger_releases", docId), {
  ...
  return (
    <AdminLayout>
      ...
      <Modal
        ...
        title={isCreating ? "Nouveau communiqué blogueur" : "Modifier le communiqué"}
        size="large"
      >
        <div className="space-y-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="isActiveRelease" className="text-sm text-gray-700">
              Actif (visible sur la page blogueurs)
            </label>
          </div>
...
export default AdminBloggerReleases;
