/**
 * AdminExpatsPromotions
 *
 * Admin page for managing expat promotions/hackathons.
...
const AdminExpatsPromotions: React.FC = () => {
  ...
  const fetchPromotions = useCallback(async () => {
    ...
    setError(null);
    try {
      const fn = httpsCallable(functionsAffiliate, "adminGetExpatPromotions");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingPromo) {
        const fn = httpsCallable(functionsAffiliate, "adminUpdateExpatPromotion");
        await fn({
        ...
        const fn = httpsCallable(functionsAffiliate, "adminCreateExpatPromotion");
        await fn({
          name: formData.name,
          description: formData.description,
          type: formData.type,
  ...
  const handleDelete = async (promoId: string) => {
    ...
    try {
      const fn = httpsCallable(functionsAffiliate, "adminDeleteExpatPromotion");
  ...
  const handleDuplicate = async (promo: Promotion) => {
    try {
      const fn = httpsCallable(functionsAffiliate, "adminDuplicateExpatPromotion");
      await fn({
...
export default AdminExpatsPromotions;
