/**
 * AdminPartnersPromotions
 *
 * Admin page for managing partner promotions/hackathons.
...
const AdminPartnersPromotions: React.FC = () => {
  ...
  const fetchPromotions = useCallback(async () => {
    ...
    setError(null);
    try {
      const fn = httpsCallable(functionsAffiliate, "adminGetPartnerPromotions");
  ...
  const handleOpenDialog = (promo?: Promotion) => {
    if (promo) {
      ...
      setFormData({
        ...
        appliesToTypes: promo.appliesToTypes || ["client_call"],
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingPromo) {
        const fn = httpsCallable(functionsAffiliate, "adminUpdatePartnerPromotion");
        await fn({
        ...
        const fn = httpsCallable(functionsAffiliate, "adminCreatePartnerPromotion");
        await fn({
          name: formData.name,
          description: formData.description,
          type: formData.type,
  ...
  const handleDelete = async (promoId: string) => {
    ...
    try {
      const fn = httpsCallable(functionsAffiliate, "adminDeletePartnerPromotion");
  ...
  const handleDuplicate = async (promo: Promotion) => {
    try {
      const fn = httpsCallable(functionsAffiliate, "adminDuplicatePartnerPromotion");
      await fn({
  ...
  const getPromoStatus = (promo: Promotion): { statusType: StatusType; label: string } => {
    ...
    if (now < start) return { statusType: "info", label: "Planifie" };
    if (now > end) return { statusType: "error", label: "Termine" };
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Gerer les hackathons et promotions avec multiplicateurs de commissions
            </p>
          </div>
                ...
                <TableBody>
                  {promotions.map((promo) => (
                    <TableRow key={promo.id}>
                      ...
                      <TableCell>
                        <span className="font-bold text-blue-600">x{promo.multiplier}</span>
                      </TableCell>
                      <TableCell>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-4">
              ...
              <div>
                <Label>Pays cibles (vide = tous)</Label>
                <Input
                ...
                <p className="text-xs text-gray-500 mt-1 flex items-center gap-1">
                  <Globe className="h-3 w-3" /> Codes ISO separes par des virgules
                </p>
              </div>
              <div>
                <Label>Budget max (cents, 0 = illimite)</Label>
                <Input
                  type="number"
                  min={0}
...
export default AdminPartnersPromotions;
