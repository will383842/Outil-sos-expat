...
const AdminInfluencerDetail: React.FC = () => {
  ...
  const handleSaveRates = async () => {
    ...
    setRatesSaving(true);
    try {
      const fn = httpsCallable<{ influencerId: string; lockedRates: Record<string, number> }, { success: boolean }>(
        functionsAffiliate,
        'adminUpdateInfluencerLockedRates'
      );
      await fn({ influencerId: id, lockedRates: ratesForm });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className={`${UI.card} p-6`}>
              ...
              {influencer.commissionPlanName && (
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
                  Plan: {influencer.commissionPlanName} {influencer.rateLockDate && `(verrouillé le ${new Date(influencer.rateLockDate).toLocaleDateString(intl.locale)})`}
                </p>
              )}
