 ...
 *
 * Admin page for managing blogger promotions/hackathons.
...
const COMMISSION_TYPES = [
  ...
  { value: "provider_recruitment", label: "Provider Recruitment ($5)" },
];
...
const AdminBloggersPromotions: React.FC = () => {
  ...
  const fetchPromotions = useCallback(async () => {
    ...
    setError(null);
    try {
      const fn = httpsCallable(functionsAffiliate, "adminGetBloggerPromotions");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingPromo) {
        const fn = httpsCallable(functionsAffiliate, "adminUpdateBloggerPromotion");
        await fn({
        ...
        const fn = httpsCallable(functionsAffiliate, "adminCreateBloggerPromotion");
        await fn({
          name: formData.name,
          description: formData.description,
          type: formData.type,
  ...
  const handleDelete = async (promoId: string) => {
    ...
    try {
      const fn = httpsCallable(functionsAffiliate, "adminDeleteBloggerPromotion");
  ...
  const handleDuplicate = async (promo: Promotion) => {
    try {
      const fn = httpsCallable(functionsAffiliate, "adminDuplicateBloggerPromotion");
      await fn({
                ...
                <TableBody>
                  {promotions.map((promo) => (
                    <TableRow key={promo.id}>
                      ...
                      <TableCell>
                        <span className="font-bold text-green-600">x{promo.multiplier}</span>
                      </TableCell>
                      <TableCell>
...
export default AdminBloggersPromotions;
