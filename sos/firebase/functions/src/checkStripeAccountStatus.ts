...
export const checkStripeAccountStatus = onCall<{
  ...
  try {
    ...
    // P0 FIX: Si le document n'existe pas dans lawyers/expats, vérifier dans sos_profiles
    // et créer le document manquant si nécessaire
    if (!userDoc.exists) {
      console.log(`[checkStripeAccountStatus] Document not found in ${collectionName}/${userId}, checking sos_profiles...`);
      ...
      if (sosProfileDoc.exists) {
        ...
        console.log(`[checkStripeAccountStatus] Creating missing document in ${collectionName}/${userId} from sos_profiles`);
        ...
        console.log(`[checkStripeAccountStatus] Creating missing document in ${collectionName}/${userId} from users`);
