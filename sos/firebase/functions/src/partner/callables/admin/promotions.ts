/**
 * Admin Callables: Partner Promotions Management
 *
 * Admin functions for managing partner hackathons and promotions:
 * - CRUD operations
 * - Stats and analytics
 * - Duplication
 */
...
export const adminGetPartnerPromotions = onCall(
  { ...partnerAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{
    ...
    try {
      ...
      logger.error("[adminGetPartnerPromotions] Error", { error });
...
export const adminCreatePartnerPromotion = onCall(
  { ...partnerAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; promotionId?: string; error?: string }> => {
    ...
    try {
      ...
      logger.error("[adminCreatePartnerPromotion] Error", { error });
...
export const adminUpdatePartnerPromotion = onCall(
  { ...partnerAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      logger.error("[adminUpdatePartnerPromotion] Error", { error });
...
export const adminDeletePartnerPromotion = onCall(
  { ...partnerAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; error?: string }> => {
    ensureInitialized();
    await assertAdmin(request);
    ...
    try {
      ...
      logger.error("[adminDeletePartnerPromotion] Error", { error });
...
export const adminGetPartnerPromotionStats = onCall(
  { ...partnerAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{
    ...
    try {
      ...
      for (const doc of commissionsQuery.docs) {
        ...
        const pid = commission.partnerId;
        const existing = participantStats.get(pid) || {
          email: commission.partnerEmail || "",
          count: 0,
          amount: 0,
        };
        existing.count++;
      ...
      logger.error("[adminGetPartnerPromotionStats] Error", { error });
...
export const adminDuplicatePartnerPromotion = onCall(
  { ...partnerAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; promotionId?: string; error?: string }> => {
    ensureInitialized();
    const adminUid = await assertAdmin(request);
    ...
    try {
      ...
      logger.error("[adminDuplicatePartnerPromotion] Error", { error });
