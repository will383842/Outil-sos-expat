...
export async function checkBloggerProviderRecruitment(
  ...
  try {
    ...
    await db.collection("blogger_notifications").add({
      ...
      message: `${providerName} s'est inscrit via votre lien. Vous gagnerez $${(config.commissionRecruitmentAmount / 100).toFixed(0)} sur chaque appel pendant ${config.recruitmentWindowMonths ?? 6} mois.`,
      messageTranslations: {
        fr: `${providerName} s'est inscrit via votre lien. Vous gagnerez $${(config.commissionRecruitmentAmount / 100).toFixed(0)} sur chaque appel pendant ${config.recruitmentWindowMonths ?? 6} mois.`,
        en: `${providerName} registered via your link. You'll earn $${(config.commissionRecruitmentAmount / 100).toFixed(0)} on each call for ${config.recruitmentWindowMonths ?? 6} months.`,
        es: `${providerName} se registró a través de tu enlace. Ganarás $${(config.commissionRecruitmentAmount / 100).toFixed(0)} por cada llamada durante ${config.recruitmentWindowMonths ?? 6} meses.`,
        de: `${providerName} hat sich über Ihren Link registriert. Sie verdienen $${(config.commissionRecruitmentAmount / 100).toFixed(0)} pro Anruf für ${config.recruitmentWindowMonths ?? 6} Monate.`,
        pt: `${providerName} registrou-se pelo seu link. Você ganhará $${(config.commissionRecruitmentAmount / 100).toFixed(0)} por cada chamada durante ${config.recruitmentWindowMonths ?? 6} meses.`,
        ru: `${providerName} зарегистрировался по вашей ссылке. Вы будете получать $${(config.commissionRecruitmentAmount / 100).toFixed(0)} за каждый звонок в течение ${config.recruitmentWindowMonths ?? 6} месяцев.`,
        hi: `${providerName} आपके लिंक से पंजीकृत हुआ। आप ${config.recruitmentWindowMonths ?? 6} महीने तक हर कॉल पर $${(config.commissionRecruitmentAmount / 100).toFixed(0)} कमाएंगे।`,
        zh: `${providerName} 通过您的链接注册。您将在${config.recruitmentWindowMonths ?? 6}个月内每次通话赚取 $${(config.commissionRecruitmentAmount / 100).toFixed(0)}。`,
        ar: `${providerName} سجّل عبر رابطك. ستحصل على $${(config.commissionRecruitmentAmount / 100).toFixed(0)} عن كل مكالمة لمدة ${config.recruitmentWindowMonths ?? 6} أشهر.`,
      },
      isRead: false,
      emailSent: false,
      data: {
    ...
    for (const recruitmentDoc of recruitmentQuery.docs) {
      ...
      if (result.success) {
        ...
        await db.collection("blogger_notifications").add({
          ...
          titleTranslations: {
            fr: "Commission recrutement !",
            en: "Recruitment commission!",
            es: "¡Comisión de reclutamiento!",
            de: "Rekrutierungsprovision!",
            pt: "Comissão de recrutamento!",
            ru: "Комиссия за рекрутинг!",
            hi: "भर्ती कमीशन!",
            zh: "招募佣金！",
            ar: "عمولة التجنيد!",
          },
          ...
          messageTranslations: {
            fr: `Vous avez gagné $${(result.amount! / 100).toFixed(2)} grâce à ${recruitment.providerName}.`,
            en: `You earned $${(result.amount! / 100).toFixed(2)} thanks to ${recruitment.providerName}.`,
            es: `Has ganado $${(result.amount! / 100).toFixed(2)} gracias a ${recruitment.providerName}.`,
            de: `Sie haben $${(result.amount! / 100).toFixed(2)} dank ${recruitment.providerName} verdient.`,
            pt: `Você ganhou $${(result.amount! / 100).toFixed(2)} graças a ${recruitment.providerName}.`,
            ru: `Вы заработали $${(result.amount! / 100).toFixed(2)} благодаря ${recruitment.providerName}.`,
            hi: `आपने ${recruitment.providerName} की बदौलत $${(result.amount! / 100).toFixed(2)} कमाए।`,
            zh: `您因 ${recruitment.providerName} 赚取了 $${(result.amount! / 100).toFixed(2)}。`,
            ar: `لقد ربحت $${(result.amount! / 100).toFixed(2)} بفضل ${recruitment.providerName}.`,
          },
          isRead: false,
          emailSent: false,
          data: {
