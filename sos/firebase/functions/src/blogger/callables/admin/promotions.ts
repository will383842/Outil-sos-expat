/**
 * Admin Callables: Blogger Promotions Management
 *
 * Admin functions for managing blogger hackathons and promotions:
 * - CRUD operations
 * - Stats and analytics
 * - Duplication
 */
...
import {
  ...
  getPromotion,
  getPromotionStats,
} from "../../services/bloggerPromotionService";
...
export const adminGetBloggerPromotions = onCall(
  ...
  async (request): Promise<{
    ...
    try {
      ...
      logger.error("[adminGetBloggerPromotions] Error", { error });
...
export const adminCreateBloggerPromotion = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; promotionId?: string; error?: string }> => {
    ...
    try {
      ...
      logger.error("[adminCreateBloggerPromotion] Error", { error });
...
export const adminUpdateBloggerPromotion = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      logger.error("[adminUpdateBloggerPromotion] Error", { error });
...
export const adminDeleteBloggerPromotion = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; error?: string }> => {
    ensureInitialized();
    await assertAdmin(request);
    ...
    try {
      ...
      logger.error("[adminDeleteBloggerPromotion] Error", { error });
...
export const adminGetBloggerPromotionStats = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{
    ...
    try {
      ...
      for (const doc of commissionsQuery.docs) {
        ...
        const pid = commission.bloggerId;
        const existing = participantStats.get(pid) || {
          email: commission.bloggerEmail || "",
          count: 0,
          amount: 0,
        };
        existing.count++;
      ...
      logger.error("[adminGetBloggerPromotionStats] Error", { error });
...
export const adminDuplicateBloggerPromotion = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; promotionId?: string; error?: string }> => {
    ensureInitialized();
    const adminUid = await assertAdmin(request);
    ...
    try {
      ...
      logger.error("[adminDuplicateBloggerPromotion] Error", { error });
