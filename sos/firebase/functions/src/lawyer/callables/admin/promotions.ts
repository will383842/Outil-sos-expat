/**
 * Admin Callables: Lawyer Promotions Management
 *
 * Admin functions for managing lawyer hackathons and promotions:
 * - CRUD operations
 * - Stats and analytics
 * - Duplication
 */
...
import {
  ...
  getPromotion,
  getPromotionStats,
} from "../../services/lawyerPromotionService";
...
export const adminGetLawyerPromotions = onCall(
  ...
  async (request): Promise<{
    ...
    try {
      ...
      logger.error("[adminGetLawyerPromotions] Error", { error });
...
export const adminCreateLawyerPromotion = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; promotionId?: string; error?: string }> => {
    ...
    try {
      ...
      logger.error("[adminCreateLawyerPromotion] Error", { error });
...
export const adminUpdateLawyerPromotion = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      logger.error("[adminUpdateLawyerPromotion] Error", { error });
...
export const adminDeleteLawyerPromotion = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; error?: string }> => {
    ensureInitialized();
    await assertAdmin(request);
    ...
    try {
      ...
      logger.error("[adminDeleteLawyerPromotion] Error", { error });
...
export const adminGetLawyerPromotionStats = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{
    ...
    try {
      ...
      for (const doc of commissionsQuery.docs) {
        ...
        const pid = commission.lawyerId;
        const existing = participantStats.get(pid) || {
          email: commission.lawyerEmail || "",
          count: 0,
          amount: 0,
        };
        existing.count++;
      ...
      logger.error("[adminGetLawyerPromotionStats] Error", { error });
...
export const adminDuplicateLawyerPromotion = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; promotionId?: string; error?: string }> => {
    ensureInitialized();
    const adminUid = await assertAdmin(request);
    ...
    try {
      ...
      logger.error("[adminDuplicateLawyerPromotion] Error", { error });
