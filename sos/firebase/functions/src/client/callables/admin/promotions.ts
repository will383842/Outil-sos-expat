/**
 * Admin Callables: Client Promotions Management
 *
 * Admin functions for managing client hackathons and promotions:
 * - CRUD operations
 * - Stats and analytics
 * - Duplication
 */
...
import {
  ...
  getPromotion,
  getPromotionStats,
} from "../../services/clientPromotionService";
...
export const adminGetClientPromotions = onCall(
  ...
  async (request): Promise<{
    ...
    try {
      ...
      logger.error("[adminGetClientPromotions] Error", { error });
...
export const adminCreateClientPromotion = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; promotionId?: string; error?: string }> => {
    ...
    try {
      ...
      logger.error("[adminCreateClientPromotion] Error", { error });
...
export const adminUpdateClientPromotion = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      logger.error("[adminUpdateClientPromotion] Error", { error });
...
export const adminDeleteClientPromotion = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; error?: string }> => {
    ensureInitialized();
    await assertAdmin(request);
    ...
    try {
      ...
      logger.error("[adminDeleteClientPromotion] Error", { error });
...
export const adminGetClientPromotionStats = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{
    ...
    try {
      ...
      logger.error("[adminGetClientPromotionStats] Error", { error });
...
export const adminDuplicateClientPromotion = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; promotionId?: string; error?: string }> => {
    ensureInitialized();
    const adminUid = await assertAdmin(request);
    ...
    try {
      ...
      logger.error("[adminDuplicateClientPromotion] Error", { error });
