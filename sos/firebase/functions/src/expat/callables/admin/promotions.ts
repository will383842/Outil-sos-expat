/**
 * Admin Callables: Expat Promotions Management
 *
 * Admin functions for managing expat hackathons and promotions:
 * - CRUD operations
 * - Stats and analytics
 * - Duplication
 */
...
import {
  ...
  getPromotion,
  getPromotionStats,
} from "../../services/expatPromotionService";
...
export const adminGetExpatPromotions = onCall(
  ...
  async (request): Promise<{
    ...
    try {
      ...
      logger.error("[adminGetExpatPromotions] Error", { error });
...
export const adminCreateExpatPromotion = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; promotionId?: string; error?: string }> => {
    ...
    try {
      ...
      logger.error("[adminCreateExpatPromotion] Error", { error });
...
export const adminUpdateExpatPromotion = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      logger.error("[adminUpdateExpatPromotion] Error", { error });
...
export const adminDeleteExpatPromotion = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; error?: string }> => {
    ensureInitialized();
    await assertAdmin(request);
    ...
    try {
      ...
      logger.error("[adminDeleteExpatPromotion] Error", { error });
...
export const adminGetExpatPromotionStats = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{
    ...
    try {
      ...
      for (const doc of commissionsQuery.docs) {
        ...
        const pid = commission.expatId;
        const existing = participantStats.get(pid) || {
          email: commission.expatEmail || "",
          count: 0,
          amount: 0,
        };
        existing.count++;
      ...
      logger.error("[adminGetExpatPromotionStats] Error", { error });
...
export const adminDuplicateExpatPromotion = onCall(
  { ...affiliateAdminConfig, memory: "256MiB", timeoutSeconds: 60 },
  async (request): Promise<{ success: boolean; promotionId?: string; error?: string }> => {
    ensureInitialized();
    const adminUid = await assertAdmin(request);
    ...
    try {
      ...
      logger.error("[adminDuplicateExpatPromotion] Error", { error });
