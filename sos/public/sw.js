...
// Enhanced precaching for PWA 100% score
...
const BACKGROUND_PRECACHE = [
  ...
  '/icons/shortcut-urgence.png',
  '/icons/shortcut-consultation.png',
  '/icons/shortcut-documents.png'
];
...
async function cacheEssentialResources() {
  ...
  // Cache background resources (non-blocking, after activation)
  setTimeout(async () => {
    console.log('[SW] Starting background precache...');
    for (const url of BACKGROUND_PRECACHE) {
      try {
        const response = await fetchWithTimeout(url, NETWORK_TIMEOUTS.images);
        if (response.ok) {
          await cache.put(url, response);
          console.log(`[SW] ✅ Background cached: ${url}`);
        }
      } catch (error) {
        // Silently fail for background resources
      }
    }
    console.log('[SW] ✅ Background precache complete');
  }, 5000); // Start after 5s to not block initial load
}
