/**
 * Script pour cocher "Appelable" (isCallable: true) sur tous les profils AAA
 *
 * Ce script met à jour le champ isCallable à true sur tous les profils AAA
 * dans les collections sos_profiles et users.
 *
 ...
 *
 * Usage: node scripts/fix-aaa-profiles-callable.js [--dry-run]
 */
...
async function fixAaaCallable() {
  console.log('='.repeat(60));
  console.log('ACTIVATION isCallable SUR TOUS LES PROFILS AAA');
  console.log('='.repeat(60));
  ...
  console.log('[3/4] Activation isCallable sur les profils AAA...');

  for (const { uid, data, ref } of aaaProfiles) {
    try {
      // Vérifier si déjà appelable
      if (data.isCallable === true) {
        console.log(`  [OK] ${uid} (${data.fullName || data.firstName || 'N/A'}) - Déjà appelable`);
      ...
      if (DRY_RUN) {
        console.log(`  [DRY-RUN] ${uid} (${data.fullName || data.firstName || 'N/A'}) - isCallable: ${data.isCallable} → true`);
      } else {
        ...
        console.log(`  [FIXED] ${uid} (${data.fullName || data.firstName || 'N/A'}) - isCallable activé`);
      }
  ...
  console.log(`Déjà appelables:        ${stats.alreadyCorrect}`);
  ...
  if (DRY_RUN) {
    ...
    console.log('\n[SUCCESS] Tous les profils AAA sont maintenant appelables.');
  }
}
