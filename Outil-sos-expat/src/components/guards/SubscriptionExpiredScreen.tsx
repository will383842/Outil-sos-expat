...
export default function SubscriptionExpiredScreen({
  ...
  // Formater la date selon la locale courante
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 via-white to-red-50 p-4">
      <div className="max-w-lg w-full">
          ...
          <CardHeader className="text-center pb-2">
            ...
            <div className="flex justify-center mb-4">
              ...
              >
                ...
                {isInGracePeriod ? t("subscriptionExpired.gracePeriod") : t("subscriptionExpired.expired")}
              </span>
            </div>
            ...
            <CardTitle className="text-2xl font-bold text-gray-900">
              {isInGracePeriod
                ? t("subscriptionExpired.titleGrace")
                : t("subscriptionExpired.titleSuspended")}
            </CardTitle>

            <CardDescription className="text-gray-600 mt-2">
              {isInGracePeriod
                ? t("subscriptionExpired.descriptionGrace", { days: gracePeriodDays! - daysSince })
                : t("subscriptionExpired.descriptionExpired")}
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-6">
            ...
            {formattedExpirationDate && (
              <div className="flex items-center gap-3 bg-gray-50 rounded-xl p-4">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-600">{t("subscriptionExpired.expirationDate")}</p>
                  <p className="font-semibold text-gray-900">{formattedExpirationDate}</p>
                </div>
                {daysSince > 0 && (
                  <span className="text-sm text-gray-500">
                    {t("subscriptionExpired.daysAgo", { count: daysSince })}
                  </span>
                )}
              </div>
            )}
            ...
            {isInGracePeriod && (
              <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4">
                ...
                <div>
                  <p className="font-medium text-amber-900">{t("subscriptionExpired.gracePeriodActive")}</p>
                  <p className="text-sm text-amber-700">
                    {t("subscriptionExpired.daysRemaining", { count: gracePeriodDays! - daysSince })}
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="bg-gradient-to-r from-red-50 to-amber-50 rounded-xl p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-medium text-gray-900">{t("subscriptionExpired.whatYouGet")}</span>
              </div>
              <ul className="space-y-2 text-sm text-gray-600">
                <li className="flex items-center gap-2">
                  <span className="w-1.5 h-1.5 bg-red-500 rounded-full" aria-hidden="true" />
                  {t("subscriptionExpired.benefitAI")}
                </li>
                <li className="flex items-center gap-2">
                  <span className="w-1.5 h-1.5 bg-red-500 rounded-full" aria-hidden="true" />
                  {t("subscriptionExpired.benefitCountries")}
                </li>
                <li className="flex items-center gap-2">
                  <span className="w-1.5 h-1.5 bg-red-500 rounded-full" aria-hidden="true" />
                  {t("subscriptionExpired.benefitSources")}
                </li>
              </ul>
            </div>
            ...
            <div className="space-y-3 pt-2">
              ...
              >
                ...
                {t("subscriptionExpired.renewButton")}
              ...
              >
                ...
                {t("subscriptionExpired.contactSupport")}
              </Button>
            ...
            {userEmail && (
              <p className="text-center text-xs text-gray-400 pt-2">
                {t("subscriptionExpired.connectedAs", { email: userEmail })}
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <p className="mt-8 text-center text-xs text-gray-400">
          {new Date().getFullYear()} SOS Expats. {t("subscriptionExpired.copyright")}
        </p>
      </div>
    </div>
  );
