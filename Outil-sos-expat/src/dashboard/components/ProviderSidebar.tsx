 ...
 *
 * Fonctionnalités :
 * - Navigation vers Accueil, Historique des conversations
 * - Sélecteur de prestataire (pour les comptes multi)
 * - Indicateur de quota IA (aiCallsUsed/aiCallsLimit synced from SOS admin)
 * - Indicateur d'abonnement
 * (Note: Profil géré sur SOS Expat, onglet supprimé)
 *
 * =============================================================================
 */
  ...
  if (providers.length <= 1) {
    return (
      <div className="px-3 py-2 text-sm text-slate-300">
        <p className="font-medium truncate">{activeProvider?.name || activeProvider?.fullName || t("provider:sidebar.select")}</p>
...
// Helper function to get nav items with translations
// Note: "Mon Profil" tab removed - not useful for SSO users, profile is managed on SOS Expat
...
export default function ProviderSidebar() {
  ...
  // Quota IA - lit depuis les champs du provider
  // Note: aiCallsLimit est défini par l'admin, aiCallsUsed est incrémenté par les appels IA
  const providerData = activeProvider as Record<string, unknown> | null;
  const aiQuotaUsed = (providerData?.aiCallsUsed as number) || 0;
  // Priorité: aiCallsLimit (défini par admin) > aiQuota (legacy) > 100 (défaut)
    ...
    <aside className="flex flex-col h-screen w-64 bg-slate-900 text-white">
      ...
      <div className="border-t border-slate-700/50 p-3 space-y-3">
        ...
        <div className="flex items-center gap-2 px-3 py-2 text-sm">
          ...
          <span className="text-slate-300">{t("provider:sidebar.subscription")}</span>
          <span className={cn(
